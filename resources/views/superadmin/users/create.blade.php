<x-superadmin-layout>
    <x-slot name="title">Tambah Siswa — Super Admin</x-slot>
    ...
    <div style="max-width:580px;">
        <div style="margin-bottom:22px;">
            ...
            <h1 style="font-size:20px;font-weight:900;color:#0f172a;margin:0 0 3px;">Tambah Akun Siswa</h1>
            <p style="font-size:13px;color:#64748b;margin:0;">Buat akun baru untuk siswa / customer kantin</p>
        </div>

        <div class="sa-card" style="padding:24px;">
            <form method="POST" action="{{ route('superadmin.users.store') }}" style="display:flex;flex-direction:column;gap:18px;">
                @csrf
                <div>
                    <label class="sa-label">Nama Lengkap</label>
                    <input type="text" name="name" value="{{ old('name') }}" class="sa-input" required placeholder="Nama siswa">
                    @error('name')<p style="font-size:11px;color:#dc2626;margin-top:4px;">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="sa-label">Email</label>
                    <input type="email" name="email" value="{{ old('email') }}" class="sa-input" required placeholder="email@sekolah.id">
                    @error('email')<p style="font-size:11px;color:#dc2626;margin-top:4px;">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="sa-label">Password</label>
                    <input type="password" name="password" class="sa-input" required placeholder="Minimal 8 karakter">
                    @error('password')<p style="font-size:11px;color:#dc2626;margin-top:4px;">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="sa-label">Konfirmasi Password</label>
