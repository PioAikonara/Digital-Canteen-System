<x-superadmin-layout>
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:22px;">
        ...
        <div>
            <h1 style="font-size:20px;font-weight:900;color:#0f172a;margin:0 0 2px;">Edit Petugas Kantin</h1>
            <p style="font-size:12px;color:#64748b;margin:0;">{{ $admin->name }}</p>
        </div>
    </div>
            ...
            <form method="POST" action="{{ route('superadmin.admins.update', $admin) }}">
                ...
                <div style="display:grid;gap:16px;">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $admin->name) }}" class="sa-input" required>
                        @error('name')<p class="sa-error">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="sa-label">Email <span style="color:#dc2626;">*</span></label>
                        <input type="email" name="email" value="{{ old('email', $admin->email) }}" class="sa-input" required>
                        @error('email')<p class="sa-error">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="sa-label">Password Baru <span style="color:#94a3b8;font-weight:500;">(kosongkan jika tidak diubah)</span></label>
                        <input type="password" name="password" class="sa-input" placeholder="Min. 8 karakter">
                        @error('password')<p class="sa-error">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="sa-label">Konfirmasi Password Baru</label>
                        <input type="password" name="password_confirmation" class="sa-input" placeholder="Ulangi password baru">
                    </div>
                </div>
